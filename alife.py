...
Press 'P' to pause/resume. Use left/right arrows to step back/forward while paused.
"""
...
class Producer:
    """
    Gains energy passively, can seed, immediately removed from the list if eaten.
    """
    next_id = 0
    ...
    def update(self, producers_list, consumers_list):
        ...
        if self.energy > PRODUCER_SEED_COST and random.random() < PRODUCER_SEED_PROB:
            ...
            if NO_SEED_UNDER_CONSUMER:
                # don't seed if a consumer is there
                occupant_here = any(c.x == nx and c.y == ny for c in consumers_list)
                if occupant_here:
                    return
    ...
    def is_dead(self):
        # Not used if we remove the producer immediately on eat, 
        # but you can keep for safety
...
class Consumer:
    """
    Genes: [speed, metabolism, vision].
    - Immediate removal of producer when eaten (pop from list).
    - One occupant per cell logic -> cell_occupied check.
    ...
    def update(self, producers_list, consumers_list):
        ...
        if direction:
            # Move speed steps or until we eat
            steps= self.speed
    ...
    def reproduce(self, consumers_list):
        ...
        baby_genes= self.mutate_genes(baby_genes)
...
def run_simulation_interactive():
    ...
    def do_simulation_step(t):
        # 1) Update producers
        #  we do not do a post-cycle cull, because we remove them immediately if eaten
        for p in producers:
    ...
    while True:
        ...
        info_str= f"t={current_step} | P={np} | C={nc} | Sp={sp:.2f} | G={gn:.2f} | Met={mb:.2f} | Vis={vs:.2f} | {status}"
